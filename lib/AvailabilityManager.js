...
// Cached per timezone — Intl.DateTimeFormat construction is comparatively
// expensive, and the timezone rarely changes across trigger() calls.
...
class AvailabilityManager {
  ...
  static trigger(device, available) {
    ...
    if (!timestampFormatters.has(tz)) {
      timestampFormatters.set(tz, new Intl.DateTimeFormat('en-GB', {
        timeZone: tz,
        dateStyle: 'short',
        timeStyle: 'medium',
      }));
    }
    const timestamp = timestampFormatters.get(tz).format(new Date());
...
class AvailabilityManagerBase {
  ...
  constructor(device, options = {}) {
    ...
    this._polling = false;             // flag to prevent concurrent polls
    ...
    // Jitter: spreads the poll's timing to prevent many devices (e.g. a
    // general power outage followed by a simultaneous reconnect) from
    // contending for the Zigbee channel at once with concurrent readAttributes calls.
  ...
  /**
   * Tries to poll the device before marking it offline.
   * Reads an attribute from the Basic Cluster (0x0000) — universal to all Zigbee devices.
   * @returns {Promise<boolean>} true if the device responded, false if it failed
   */
  async _pollDevice() {
    ...
    this._polling = true;
    try {
      // Waits a random delay (0..pollJitterMaxMs) before the actual poll.
      // Reduces the chance of several devices contending for the Zigbee
      // channel at the same time when they time out on the same watchdog tick.
      if (this._pollJitterMaxMs > 0) {
        const jitter = Math.floor(Math.random() * this._pollJitterMaxMs);
        await new Promise(resolve => setTimeout(resolve, jitter));
      }

      // this.device.homey.zigbee.getNode() returns the raw Homey.ZigBeeNode
      // manager object, which has no .endpoints/.clusters — that structure
      // only exists on the ZCLNode wrapper the driver already built and
      // stored on the device during onInit.
      const basicCluster = this.device.zclNode?.endpoints?.[1]?.clusters?.basic;
      if (!basicCluster) {
        this.device.error('[Availability] Poll failed: no basic cluster on endpoint 1');
        return false;
      }

      this.device.log('[Availability] Polling device before marking offline...');

      // readAttributes with a short timeout. Whichever side loses the race
      // is defused so it can't leave a pending timer, or an unhandled
      // rejection, behind once it eventually settles.
      let timeoutId;
      const timeoutPromise = new Promise((_, reject) => {
        timeoutId = setTimeout(() => reject(new Error('Poll timeout')), this._pollTimeoutMs);
      });
      const readPromise = basicCluster.readAttributes(['manufacturerName'])
        .finally(() => clearTimeout(timeoutId));
      readPromise.catch(() => {}); // avoid unhandled rejection if timeout wins the race
...
class AvailabilityManagerPassive extends AvailabilityManagerBase {
  ...
  async _installHandleFrameHook() {
    ...
    // Node is shared by all sibling gangs of a multi-endpoint device — every
    // instance must register here so a single frame refreshes last_seen_ts
    // for ALL gangs, not just the one that happened to install the hook.
    if (!node._availabilityManagers) {
      node._availabilityManagers = [];
    }
    node._availabilityManagers.push(this);
    ...
    const notifySiblings = (endpointId, clusterId, frame) => {
      for (const mgr of node._availabilityManagers) {
        mgr._logBasicFrameNeedingResponse(endpointId, clusterId, frame);
        mgr._recordMessage(`ep${endpointId}:0x${clusterId.toString(16)}`);
        ...
        mgr._markAlive(`ep${endpointId} cl:0x${clusterId.toString(16)}`)
          .catch(e => mgr.device.error('[Availability] handleFrame hook error:', e.message));
      }
    };

    if (typeof original !== 'function') {
      node.handleFrame = (endpointId, clusterId, frame, meta) => {
        notifySiblings(endpointId, clusterId, frame);
        return false;
      };
    } else {
      node.handleFrame = (endpointId, clusterId, frame, meta) => {
  ...
  async _cleanup() {
    // Deregister this instance; only tear down the shared hook once no
    // sibling gang still depends on it.
    if (this._zigbeeNode) {
      const node = this._zigbeeNode;
      if (node._availabilityManagers) {
        node._availabilityManagers = node._availabilityManagers.filter(mgr => mgr !== this);
      }

      if (!node._availabilityManagers || node._availabilityManagers.length === 0) {
        if (this._originalHandleFrame !== null) {
          node.handleFrame = this._originalHandleFrame;
        }
        node._availabilityHookInstalled = false;
