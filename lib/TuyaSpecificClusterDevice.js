...
class TuyaSpecificClusterDevice extends ZigBeeDevice {
  ...
  async _teardown() {
    if (this._availabilityStatsInterval) {
      this.homey.clearInterval(this._availabilityStatsInterval);
      this._availabilityStatsInterval = null;
    }
    await this._availability?.uninstall().catch(() => {});
  }

  /**
   * Install passive availability tracking (main device only — the manager
   * cascades to siblings) plus an hourly log of frame counts per cluster.
   */
  async _installAvailability() {
    const { HEARTBEAT_FAST_MS, AVAILABILITY_STATS_LOG_MS } = require('./constants');
    this._availability = new AvailabilityManagerPassive(this, { timeout: HEARTBEAT_FAST_MS });
    await this._availability.install();

    this._availabilityStatsInterval = this.homey.setInterval(() => {
      const s = this._availability?.getMessageStats();
      if (!s) return;
      const idleMin = s.lastMessageAt ? Math.round((Date.now() - s.lastMessageAt) / 60000) : null;
      const bySource = Object.entries(s.bySource)
        .sort((a, b) => b[1] - a[1])
        .map(([src, n]) => `${src}=${n}`)
        .join(' ');
      this.log(`[Availability] frames last hour: ${s.previousHour} | 24h: ${s.last24h} `
        + `| last: ${idleMin ?? '—'}min ago | 24h by source: ${bySource || 'none'}`);
    }, AVAILABILITY_STATS_LOG_MS);
  }
}
